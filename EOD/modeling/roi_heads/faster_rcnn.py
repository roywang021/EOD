...
class EvidentialOutputLayers(nn.Module):
    ...
    def _dequeue_and_enqueue(self, feat, gt_classes, ious, iou_thr=0.7):
        ...
        fea_cpu = torch.Tensor.cpu(feat).detach().numpy()
        label_cpu=torch.Tensor.cpu(gt_classes).detach().numpy()
        with open('fea.txt', 'ab') as f:
            np.savetxt(f, fea_cpu, fmt='%f', delimiter=' ', newline='\r')
            f.write(b'\n')
        with open('label.txt', 'ab') as t:
            np.savetxt(t, label_cpu, fmt='%d', delimiter=' ', newline='\r')
            t.write(b'\n')
    ...
    def evidence_loss(self, evidence, target, losstype, *, reduction="mean", **kwargs):
        """
        evidence:(Tensor) �C (N, C) where C = number of classes(=21=20+background class)  !!!:evidence>=0
        target:(Tensor) �C (N) where each value is 0~C-1

        type=/ "ML":TYPE 2 Maximum Likelihood / "CE": Average Cross-entropy Loss / "SQ": Average Squares Loss

        returns 0 (instead of nan) for empty inputs.
        """
    ...
    def new_kl_divergence(self,alpha, y,num_classes):
        ...
        first_term = (
                ...
                - torch.lgamma(regularization_alpha.sum(dim=1, keepdim=True))  # K
        )
